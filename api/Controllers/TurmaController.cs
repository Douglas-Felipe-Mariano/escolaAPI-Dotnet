...
{   
    ...
    public async Task<IActionResult> ObterTurmasPorIdA(int id)
    {
        var turma = await _turmaService.ObterTurmaPorIdAsync(id);
        return turma == null ? NotFound("Turma não encontrada.") : Ok(turma);
    }

    [HttpPut("{id}")]
    public async Task<IActionResult> Atualizar(int id, [FromBody] TurmaRequestDTO request)
    {
        var sucesso = await _turmaService.AtualizarAsync(id, request);
        return sucesso ? NoContent() : NotFound("Turma não encontrada.");
    }
