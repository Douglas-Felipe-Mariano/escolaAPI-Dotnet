...
public async Task<IEnumerable<TurmaResponseDTO>> ListarTodasAsync()
    ...
    public async Task<TurmaResponseDTO?> ObterTurmaPorIdAsync(int id)
    {
        using var connection = new SqlConnection(_connectionString);
        var sql = "SELECT * FROM Turmas WHERE Id = @Id";
        var turma = await connection.QueryFirstOrDefaultAsync<Turma>(sql, new { Id = id });
        
        return turma?.ToDTO();
    }

    public async Task<bool> AtualizarAsync(int id, TurmaRequestDTO request)
    {
        using var connection = new SqlConnection(_connectionString);
        var sql = "UPDATE Turmas SET Nome = @Nome WHERE Id = @Id";
        var rowsAffected = await connection.ExecuteAsync(sql, new { request.Nome, Id = id });
        return rowsAffected > 0;
    }
}